...
async function boot() {
  ...
  if (!app.devices.some((device) => device.label && !/^Input \d+$/.test(device.label))) {
    toast('Click ⟳ beside "Input" to grant microphone access and list your Windows devices.');
  }
}
...
async function ensureInput() {
  ...
  await refreshDevices({ prompt: false });
  if (app.input.deviceId) dom.deviceSelect.value = app.input.deviceId;
}
